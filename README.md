...
![서비스 아키텍처](./public/images/EC2_Route53.png)

### 구성 설명
- **Client**: 사용자가 웹 브라우저를 통해 서비스에 접속
- **Route53**: 도메인 네임 시스템(DNS)으로 클라이언트 요청을 EC2로 라우팅
- **Elastic IP**: 고정 퍼블릭 IP로 EC2 인스턴스를 외부와 안정적으로 연결
- **EC2**: Node.js + Express 서버가 구동되는 애플리케이션 서버
- **DB**: 서비스 데이터를 저장하는 데이터베이스 (MySQL 사용)
